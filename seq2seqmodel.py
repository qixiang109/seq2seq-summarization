...
class Model(object):
    ...
    def __init__(self, vocab_size=4000, buckets=[(140,30)], size=128,
        ...
        if not forward_only:
             ...
             opt = tf.train.AdamOptimizer(learning_rate = settings.learning_rate,beta1=settings.beta1,beta2=settings.beta2,epsilon=settings.epsilon)
             for b in xrange(len(self.buckets)):
                 gradients = tf.gradients(self.losses[b], params)
                 clipped_gradients, norm = tf.clip_by_global_norm(gradients,settings.max_gradient_norm)
    ...
    def decode(self,session,sentence):
        ...
        输出:wid 表示的target句子
        """
        paded_source_wids, paded_target_wids, bucket_id = data_utils.format_source_target(sentence,[],True)
        batch_size = self.batch_size
        self.batch_size = 1
        encoder_inputs, decoder_inputs,target_weights = self.transpose_batch([(paded_source_wids, paded_target_wids)],bucket_id)
        _, _, output_logits = self.step(session, encoder_inputs, decoder_inputs, target_weights, bucket_id, True)
        self.batch_size = batch_size
    ...
    def transpose_batch(self, batch_data, bucket_id):
